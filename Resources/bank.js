...
function doBank() {
bankData = [];
var rowH = Ti.UI.createTableViewRow({hasDetail:true,});
var labelH = Titanium.UI.createLabel({text:'Bank        Account No.             Del', 
left:5,  font:{fontSize:22,fontWeight:'bold'}, color:'blue', width:'auto'});
rowH.add(labelH);
bankData.push(rowH);
...
while (rows.isValidRow()) {
    var row = Titanium.UI.createTableViewRow({
        className:'row', height:'40', width:'90%',title:rows.fieldByName('name'), 
        id:rows.fieldByName('id'), color:'#fffced', backgroundColor:'#fffced', });
    var bankLabel = Titanium.UI.createLabel({
        text:rows.fieldByName('name'), width:'40%', height:'auto', top:5, left:10,
    ...
    var accountLabel =  Titanium.UI.createLabel({
        text:rows.fieldByName('account'), width:'40%', top:5, //left:60,
    ...
    checkbox = Ti.UI.createSwitch({
        ...
        value:false, height:20, color:'blue', top:5, right:20, //width:25,
...
var bHelpBtn = Titanium.UI.createButton({top: '5%', left: '40%', width:100, 
	height:30, title: "Help", borderRadius:20,
...
var bbackBtn = Titanium.UI.createButton({top: '5%', left: '75%', width:100, 
	...
	id: 'D', backgroundColor: 'green', backgroundSelectedColor: 'blue', opacity:0.5});
bview.add(baddBtn); bview.add(bbackBtn); bview.add(bHelpBtn);
...
bHelpBtn.addEventListener('click', function(e) {});
